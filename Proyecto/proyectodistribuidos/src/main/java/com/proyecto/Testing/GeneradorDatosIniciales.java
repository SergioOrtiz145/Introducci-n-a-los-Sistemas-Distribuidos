    ...
    private static List<String> generarPrestamos(List<String> libros, int cantidad, String sede) {
        ...
        while (prestamosCreados < cantidad && intentos < cantidad * 3) {
            ...
            if (vecesPrestado < ejemplaresTotal) {
                ...
                int renovaciones = random.nextDouble() < 0.7 ? 0 : 
                                 (random.nextDouble() < 0.67 ? 1 : 2);
                
                String prestamoActivo = "true";
                
                // Formato CSV: idPrestamo,isbn,usuario,fechaPrestamo,renovaciones,prestamoActivo
                prestamos.add(String.format("%s,%s,%s,%s,%d,%s",
                    idPrestamo, isbn, usuario, 
                    fechaPrestamo.format(FORMATTER), 
                    renovaciones, prestamoActivo));
